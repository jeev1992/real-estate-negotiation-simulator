...
The `SESSION_REGISTRY` maintains references to `SellerAgentADK` instances keyed by session ID. Each agent stores its LLM conversation history in `llm_messages`, which we filter and return.
