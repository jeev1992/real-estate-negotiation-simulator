...
def _enforce_seller_tool_allowlist(tool: BaseTool, args: dict, tool_context: ToolContext):
    """before_tool_callback — block any tool not on the seller allowlist."""
    if tool.name not in _SELLER_ALLOWED_TOOLS:
        return {"error": f"tool '{tool.name}' is not authorized for the seller agent"}
    return None
...
#     What gets sent to the buyer over the A2A HTTP boundary.
...
class SellerEnvelope(BaseModel):
    """The structured message sent to the buyer (over the A2A HTTP boundary).
...
class SellerAgentADK:
    ...
    async def __aenter__(self) -> "SellerAgentADK":
        ...
        # before_tool_callback enforces the seller's tool allowlist so the
        # model cannot stray to tools outside its authorized set.
        self._agent = LlmAgent(
            ...
            tools=all_tools,
            before_tool_callback=_enforce_seller_tool_allowlist,
        )
