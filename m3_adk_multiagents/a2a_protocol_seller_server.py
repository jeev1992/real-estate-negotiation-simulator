...
from m3_adk_multiagents.seller_adk import SellerAgentADK
    ...
    return AgentCard(
        ...
        skills=[
            ...
            AgentSkill(
                ...
                description="Returns the per-round state deltas for an active session.",
                tags=["observability", "history", "a2a"],
                examples=["GET /history/{session_id}"],
...
class SellerADKA2AExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            # Emit a 'working' update so streaming clients see progress before
            # the (potentially slow) LLM + MCP roundtrip completes. Sync
            # clients ignore intermediate updates; streaming clients render them.
            await updater.update_status(
                TaskState.working,
                message=updater.new_agent_message(
                    parts=[TextPart(text=f"Seller processing round {parsed_buyer.round} offer at ${parsed_buyer.price or 0:,.0f}")],
                    metadata={"protocol": "a2a", "phase": "thinking"},
                ),
            )
            ...
            # Attach the structured negotiation summary as a durable Artifact.
            # Artifacts persist on the Task and can be fetched later, distinct
            # from the in-line message stream.
            await updater.add_artifact(
                parts=[Part(root=DataPart(data=response_payload))],
