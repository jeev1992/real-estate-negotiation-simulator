...
Modify the negotiation orchestrator to **detect when the negotiation has stalled** — three or more consecutive rounds where the offer moves less than $1,000 — and **escalate early** to terminate the loop. This builds the production pattern for *non-progressing agent detection*.
...
2. Add a *stall check*: if the last 3 entries in `offer_history` have **price movement < $1,000 across all three**, set `escalate=True` (loop exits early) and write `state['stall_reason']` for visibility.
...
5. Test with a no-ZOPA scenario:
   - Buyer instruction caps at $440K
   - Seller floor stays at $445K (the canonical floor from the inventory server)
   - The agents should converge on near-identical offers and your callback should fire at round 3 or 4
...
The stall threshold ($1K, 3 rounds) is hardcoded. In production, what would you parametrize and why?
